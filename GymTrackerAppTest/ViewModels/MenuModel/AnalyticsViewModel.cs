...
{
    private readonly MeasurementService _dataService;
    ...
    {
        _dataService = new MeasurementService();
        LoadData();
    }
    ...
    private void AddMeasurement()
    {
        if (double.TryParse(NewWeight, out double parsedWeight))
        {
            ...
            {
                UserLogin = currentUser?.Login ?? string.Empty,
                Date = DateTime.Now,
                Weight = parsedWeight,
                Chest = parsedChest,
                Biceps = parsedBiceps,
            ...
            _dataService.AddMeasurement(newItem);
            CalculateProgress();
            UpdateChart();
    ...
    private void DeleteMeasurement()
    {
        if (SelectedMeasurement != null)
        {
            ...
            _dataService.DeleteMeasurement(SelectedMeasurement);
            CalculateProgress();
            UpdateChart();
        }
    }
        ...
        WeightSeries = new ISeries[]
        {
            new LineSeries<double>
            {
                ...
                Fill = new LinearGradientPaint(
                    new[] { new SKColor(255, 140, 0, 150), new SKColor(255, 140, 0, 0) },
